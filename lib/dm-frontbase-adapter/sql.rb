class FrontbaseAdapter
  module SQL
    ...
    def comparison_statement(comparison, repository)
      ...
      column_name = quote_name(property_to_column_name(subject, repository))
    ...
    def columns_statements(properties, repository)
      properties.map {|property| quote_name(property_to_column_name(property, repository))}.join(', ')
    end
